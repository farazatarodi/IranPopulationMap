...
write_f = cwd + '\\src\\amarCSV\\coords_bakhsh.csv'
...
with open(read_f, 'r', encoding='utf-8') as read_obj, \
    ...
    # header data
    header = ['ostan', 'shahrestan', 'bakhsh',
              'fam', 'pop', 'm', 'f', 'lat', 'lon']
    ...
    for row in csv_reader:
        if rowCount > 0:
            if row[3].strip() == '' and row[2].strip() != '':
                row = row[0:3] + row[5:]

                # constructing the request url
                url = 'https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&limit=1&country=Iran&state={}&q=شهرستان {} بخش {}'.format(
                    row[0], row[1], row[2])
                r = requests.get(url)
